...
def create_app():
    ...
    # When running as EXE, data lives next to the executable
    data_dir = os.environ.get('VAULT_DATA_DIR', os.path.join(basedir, '..', 'data'))
    app.config['SECRET_KEY'] = os.environ.get('SECRET_KEY', 'change-this-in-production-please')
    app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///' + os.path.join(data_dir, 'finance.db')
    app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False
    app.config['UPLOAD_FOLDER'] = os.path.join(basedir, 'static', 'uploads')
    app.config['DOCS_FOLDER']   = os.path.join(data_dir, 'documents')
...
def _init_default_data():
    ...
    setting_defaults = [
        ...
        ('session_timeout', '30'),  # days - 0 means browser session only
    ]
